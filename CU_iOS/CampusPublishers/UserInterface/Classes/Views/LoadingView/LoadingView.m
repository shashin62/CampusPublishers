...
//
+ (id)loadingViewInView:(UIView *)aSuperview withTitle:(NSString*)inTitle
{
	...
	CGRect labelFrame = CGRectMake(0.0, 0.0, DEFAULT_LABEL_WIDTH, DEFAULT_LABEL_HEIGHT);
	UILabel *loadingLabel =
		[[UILabel alloc]
			initWithFrame:labelFrame];
	loadingLabel.text = inTitle;
	loadingLabel.tag = 101;
	
	loadingLabel.textColor = [UIColor whiteColor];
	loadingLabel.backgroundColor = [UIColor clearColor];
	loadingLabel.textAlignment = NSTextAlignmentCenter;
	loadingLabel.font = [UIFont fontWithName:@"ArialMT" size:[UIFont labelFontSize]]; //--font
...
- (void)remove:(id)obj
{
	//[[self layer] addAnimation:(CATransition*)obj 
...
-(void)setTitle:(NSString*)inTitle
{
	...
	for(NSInteger loop = 0; loop < count; loop++) 
	{
		id sv = [subViews objectAtIndex:loop];
		if([sv tag] == 101)
		{
			[(UILabel*)sv setText:inTitle];
			break;
		}
	}
}
