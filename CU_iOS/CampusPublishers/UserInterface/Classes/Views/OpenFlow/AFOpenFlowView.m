...
const static CGFloat kReflectionFraction = 0.40;
...
- (void)setUpInitialState 
{
	...
	//self.layer.position=CGPointMake(self.frame.size.width / 2, self.frame.size.height / 1.5);
...
- (void)layoutCover:(AFItemView *)aCover selectedCover:(int)selectedIndex animated:(Boolean)animated  
{
	...
	newPosition.y = halfScreenHeight + aCover.verticalPosition + 35.0;
...
@synthesize dataSource, viewDelegate, defaultImage, numberOfImages;
@synthesize selectedCover;
...
//    {
//        isDraggingACover = NO;
//        int newCover = selectedCover;//selectedCoverView.number;
//        CGPoint newPoint = [[touches anyObject] locationInView:self];
//        CGPoint lastPoint = [[touches anyObject] previousLocationInView:self];
//        if(newPoint.x - lastPoint.x < 0)
//        {
//            newCover++;
//            if(newCover == self.numberOfImages)
//            {
//                newCover = self.numberOfImages - 1;
//            }
//        }
//        else
//        {
...
- (void)centerOnSelectedCover:(BOOL)animated 
{
	...
	[scrollView setContentOffset:selectedOffset animated:NO];
}
...
{
	return (AFItemView *)[onscreenCovers objectForKey:[NSNumber numberWithInt:coverIndex]];
}
